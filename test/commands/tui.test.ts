...
describe('tui command', () => {
  ...
  it('creates copyable command suggestions with quoted arguments', () => {
    ...
    expect(commands.find((item) => item.id === 'inspect-status')?.command).to.equal("vagent tui --project 'demo project' --action status --workspace 'workspace dir'")
    expect(commands.find((item) => item.id === 'inspect-quality-details')?.command).to.equal("vagent tui --project 'demo project' --action quality --quality-details --json --workspace 'workspace dir'")
    expect(commands.find((item) => item.id === 'verify-artifacts')?.command).to.equal("vagent tui --project 'demo project' --action verify --workspace 'workspace dir'")
