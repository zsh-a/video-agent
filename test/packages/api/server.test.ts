...
describe('api server handler', () => {
  ...
  it('serves the Web Studio shell', async () => {
    ...
    expect(html).to.include('id="status-summary"')
    expect(html).to.include('id="quality-summary"')
    expect(html).to.include('id="render-summary"')
    expect(html).to.include('id="artifact-count"')
    expect(html).to.include('id="trace-count"')
    expect(html).to.include('id="provider-call-count"')
    expect(html).to.include('id="artifact-preview"')
    expect(html).to.include('id="visual-samples"')
    expect(html).to.include('Keyframes')
    expect(html).to.include('id="render-result-summary"')
    expect(html).to.include('id="render-result-actions"')
    expect(html).to.include('id="render-result-player"')
    expect(html).to.include('renderRenderResult(status.summary.render)')
    expect(html).to.include('Open video')
    ...
    expect(html).to.include('id="quality-issue-summary"')
    expect(html).to.include('id="quality-issues"')
    expect(html).to.include('id="llm-traces"')
    expect(html).to.include('/projects/" + encodeURIComponent(state.projectId) + "/provider-report')
    expect(html).to.include('/projects/" + encodeURIComponent(state.projectId) + "/quality?details=true')
    expect(html).to.include('renderLLMTraces(providerReport)')
    expect(html).to.include('renderQualityDetails(qualityDetails)')
    ...
    expect(html).to.include('id="operations-enabled"')
    expect(html).to.include('id="operation-lock-status"')
    expect(html).to.include('Enable project operations')
    expect(html).to.include('runControlledAction("Render"')
    expect(html).to.include('runControlledAction("Export"')
    expect(html).to.include('runControlledAction("Rerun"')
  ...
  it('serves project files from inside the project directory', async () => {
    ...
    try {
      ...
      await writeFile(join(root, 'projects', 'demo', 'renders', 'final.mp4'), '0123456789')

      const fetch = createApiFetchHandler({workspaceDir: root})
      const response = await fetch(new Request('http://localhost/projects/demo/files?path=renders%2Freview%2Findex.html'))
      const range = await fetch(new Request('http://localhost/projects/demo/files?path=renders%2Ffinal.mp4', {
        headers: {
          range: 'bytes=2-5',
        },
      }))
      ...
      expect(range.headers.get('accept-ranges')).to.equal('bytes')
      expect(range.headers.get('content-range')).to.equal('bytes 2-5/10')
      expect(range.headers.get('content-type')).to.equal('video/mp4')
      expect(await range.text()).to.equal('2345')
