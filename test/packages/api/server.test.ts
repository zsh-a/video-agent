...
describe('api server handler', () => {
  ...
  it('serves the Web Studio shell', async () => {
    ...
    const scriptPath = /<script[^>]+src="([^"]+\.js)"/u.exec(html)?.[1]
    const stylePath = /<link[^>]+href="([^"]+\.css)"/u.exec(html)?.[1]
    ...
    expect(stylePath).to.be.a('string')

    const scriptResponse = await fetch(new Request(`http://localhost${scriptPath}`))
    const script = await scriptResponse.text()
    const styleResponse = await fetch(new Request(`http://localhost${stylePath}`))
    const style = await styleResponse.text()

    expect(scriptResponse.status).to.equal(200)
    expect(scriptResponse.headers.get('content-type')).to.equal('text/javascript; charset=utf-8')
    expect(script).to.include('/projects/')
    expect(script).to.include('/provider-env')
    expect(script).to.include('/provider-test')
    expect(script).to.include('checkpoint_invalid')
    expect(script).to.include('export_quality_failed')
    expect(script).to.include('Enable project operations')
    expect(script).to.include('Render Result')
    expect(script).to.include('LLM Traces')
    expect(script).to.include('audio.voiceover.missing')
    expect(styleResponse.status).to.equal(200)
    expect(styleResponse.headers.get('content-type')).to.equal('text/css; charset=utf-8')
    expect(style).to.include('.panel')
  })
