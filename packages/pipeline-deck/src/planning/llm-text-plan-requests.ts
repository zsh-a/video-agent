  ...
  const baseMessage: LLMMessage = {
    content: JSON.stringify({
      ...
      instructions: [
        ...
        'Dense output-template code.text must fit target.maxSlideCharacters together with title and points: use compact "Field: input + rule/condition" lines, keep points empty or one short caption, and avoid parenthetical examples on every line.',
        ...
        'For scoring slides, visible fields must show how to assign scores: include the scale meaning such as 1=weak/5=strong when the source uses a 1-5 scale, the concrete scoring dimensions, and how score bands change priority or next action. If the source requires more than four scoring dimensions or a scale/action mapping, do not use chart; use code.text or process.steps so every dimension and decision rule is visible.',
        ...
        },
        {
          content: JSON.stringify({
            ...
            instructions: [
              ...
              'For scoring-detail repairs, the returned visible fields must include the score scale meaning, named dimensions, and a score-band or evidence-state mapping to priority, watchlist, small test, add, reduce, or exit. If a chart cannot display every required scoring dimension plus the decision mapping, switch the slide to code or process and fill the matching structured data.',
              ...
              'For output-template practical-detail repairs, rewrite code.text so each section has the section name plus a short field-specific instruction, filled example, required input, or validation condition; a headings-only code block or symbol-only field rule is still invalid.',
              'For dense output-template repairs, keep the whole slide under target.maxSlideCharacters by using compact "Field: input + rule/condition" lines, removing parenthetical examples first, and keeping points empty or one short caption.',
