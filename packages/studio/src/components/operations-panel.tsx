import type {ActionState, ExportOptions, RenderOptions, StageSummary} from '../types'
  ...
  return (
    <Panel title="Controlled Operations" summary="Studio opens in read-only review mode. Enable project operations before rerun, render, or export.">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button className="btn" type="button" onClick={props.onProviderTest}>Provider test</button>
      </div>
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="form-label">Export format
          <select className="field" value={props.exportOptions.format ?? ''} onChange={(event) => props.onExportOptionsChange({...props.exportOptions, format: event.currentTarget.value === '' ? undefined : event.currentTarget.value as 'bundle' | 'video'})}>
