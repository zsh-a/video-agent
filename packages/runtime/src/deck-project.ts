...
import type {QualityIssue, RenderedMediaQualityResult, SubtitleQualityResult, VisualFrameSample, VisualSmokeQualityResult} from '@video-agent/quality'
...
const DEFAULT_DECK_REVIEW_FRAME_CONCURRENCY = 4
...
async function createDeckHtmlFinalRenderProject(options: CreateDeckFinalRenderProjectOptions): Promise<CreateDeckFinalRenderProjectResult> {
  ...
  try {
    ...
    const artifactPath = await workspace.store.writeJson('render-output.json', {
      ...
      rendered,
      reviewHtmlPath: toProjectPath(workspace.projectDir, review.htmlPath),
      reviewReportPath: toProjectPath(workspace.projectDir, review.reportPath),
...
interface DeckReviewArtifacts {
  htmlPath: string
  report: DeckReviewReportArtifact
  reportPath: string
}

interface DeckReviewReportArtifact {
  duration: number
  format: DeckFormat
  generatedAt: string
  keyframeQualityPath: string
  ...
  title: string
  version: 1
  videoRenderer: CreateDeckFinalRenderProjectResult['videoRenderer']
}

interface DeckReviewSlideReport {
  duration: number
  end: number
  index: number
  keyframe?: {
...
async function writeDeckReviewArtifacts(input: {
