...
adapters/
  claude-code-skill/      # Claude Code skill adapter over CLI/MCP
...
bun run dev tui --action projects
bun run dev tui --project <projectId> --action status
bun run dev tui --project <projectId> --action artifact --artifact quality-report.json
bun run dev tui --project <projectId> --action verify
...
bun run dev tui --project <projectId> --action quality
bun run dev tui --project <projectId> --action quality --quality-details --json
bun run dev tui --project <projectId> --action events --event-kind provider --event-provider-status failed
bun run dev tui --project <projectId> --action rerun --from-stage script
bun run dev tui --project <projectId> --action audio --no-render-audio
bun run dev tui --project <projectId> --action visual
bun run dev tui --project <projectId> --action visual --visual-include-content
bun run dev tui --project <projectId> --action render --render-renderer hyperframes --render-output ./hyperframes-render
bun run dev tui --project <projectId> --action export --export-format hyperframes --export-clean-output --export-output ./hyperframes-render
...
需要让 agent client 只用显式 provider 变量做健康检查时，可以重复传 `--env KEY=VALUE`：

```sh
bun run dev doctor --env VIDEO_AGENT_ASR_URL=https://provider.example/asr --json
```
...
```text
ingest -> provider understand -> evidence-backed plan -> clip-plan-aligned script -> provider voiceover -> quality report
```
...
`quality-report.json` 会检查 storyboard scene/sourceRange timing、clip plan sourceRange 是否越界、同源 sourceRange 是否重叠或跳段、clip duration 是否匹配 sourceRange、clip plan 是否和 timeline video items 对齐、timeline 越界、narration start/duration 完整性、narration 重叠或越界、TTS 是否覆盖每个 narration、TTS duration 是否明显偏离 narration timing，以及 TTS 是否引用未知 narration。ASR/VLM/TTS provider 产物会在写入 artifact 前通过共享 schema 校验。
...
如果最终视频包含视频流，`render-output.json` 还会包含 `visualQuality`。它通过 `ffmpeg blackdetect` 读取黑屏片段和黑屏占比，并抽取 first/middle/end 三张缩略图样本；`frameSamples` 保存完整样本列表，`frameSample` 保留第一张样本作为兼容字段。每张成功样本会记录 `sha256` 内容指纹；多个采样点完全一致会写入 static-frame warning，三点样本的 JPEG 字节大小几乎无变化时会写入 low-variation warning。黑屏占比较高会写入 warning，几乎全黑会写入 error，缩略图抽样失败会写入 warning，并纳入项目级 `status` / `quality` 聚合。
...
`worker` 会扫描 workspace 内 failed/running 的本地 job，从第一个 failed/running/pending stage 恢复执行。可以用 `--dry-run` 查看将恢复哪些项目，用 `--status failed|running|active` 过滤状态，用 `--limit` 控制本轮恢复数量，用 `--order-by oldest|recent|attempt` 控制恢复候选排序，用 `--max-attempts` 跳过已经达到 stage attempt 上限的 job。处理 running job 时，可以用 `--running-stale-after-ms <ms>` 跳过最近仍有更新的任务，避免本地 worker 抢占仍在运行的进程。worker 会在恢复前预检 checkpoint artifacts；缺失、变更、未纳入 manifest 或 IR/provider schema 无效的前置 artifact 会被跳过，并返回 artifact 问题列表。跳过结果会带 `skipReason`，例如 `attempt-limit`、`checkpoint-invalid`、`limit`、`running-active` 或 `not-recoverable`。CLI/TUI 的非 JSON 输出会展示 missing、changed、schema invalid、untracked 和 schema validation issue，便于直接定位坏文件。

`tui` 会输出轻量终端 dashboard，复用 runtime 的 project/status/events/artifacts 接口展示当前 workspace、最近更新项目、stage 状态、质量摘要、render 摘要、artifact integrity 摘要、artifact 列表、最近事件和下一步可复制命令。默认渲染一次；需要持续刷新时可以传 `--watch`，也可以用 `--project <projectId>` 固定查看某个项目。需要从 TUI 入口触发受控操作时，可以用 `--action projects` 列出 workspace 项目，用 `--action status` 输出项目 job/provider/quality/render 摘要，用 `--action artifact --artifact <name>` 检查 artifact，用 `--action verify` 校验 artifact manifest、内容 hash 和已知 IR/provider schema，用 `--action commands` 单独输出命令建议，用 `--action select` 进入编号/ID guided selector，用 `--action provider-test --provider-role asr|vlm|tts|all` 验证 provider contract，用 `--action quality --quality-details` 检查项目可交付质量和可选原始质量 artifacts，用 `--action events --event-kind pipeline|provider --event-stage <stage> --event-type <type> --event-provider-role asr|vlm|tts --event-provider-status failed|succeeded` 读取过滤后的 pipeline/provider 事件，用 `--action rerun --from-stage <stage>` 重跑项目，用 `--action audio` 预检音频输入和 voiceover alignment，用 `--action visual --visual-include-content` 检查渲染缩略图样本元数据和可选 base64 内容，用 `--action render --render-renderer ffmpeg|hyperframes --render-output <path>` 渲染项目，用 `--action export --export-format video|hyperframes|bundle --export-output <path> --export-clean-output --export-require-quality` 导出项目，或用 `--action worker --dry-run|--status|--limit|--max-attempts` 恢复 workspace job。TUI rerun action 遇到 checkpoint artifact 问题时会保留 dashboard 输出，非 JSON 输出列出 artifact 问题，`--json` 返回 `type: "checkpoint-error"` 和 `error.code: "checkpoint_invalid"`；TUI export action 默认启用质量门禁，失败时保留 dashboard 输出，并返回 `type: "export-quality-error"` / `error.code: "export_quality_failed"`。命令建议默认使用 `bun run dev` 前缀，可以通过 `--command-prefix vagent` 改成已安装 CLI 的形式；`--action commands --json` 和 `--action select --json` 会返回 `id`、`category`、`description`、`priority` 和 `command`，包含 quality details、artifact verify、audio preflight、visual samples、render、quality-gated video export 和 clean HyperFrames directory export 等诊断/交付动作，方便未来 TUI/Agent 客户端复用同一套 guided action 元数据。
...
`video_agent_doctor`、`video_agent_provider_env` 和 `video_agent_provider_test` 都支持 `env` object 参数，传入后只使用这些显式变量做 provider 检查。
...
如果 checkpoint artifacts 不完整或 schema 无效，HTTP API 会返回 `409`，并在 `error.missingArtifacts`、`error.changedArtifacts`、`error.untrackedArtifacts`、`error.schemaInvalidArtifacts` 中列出问题文件。
...
- `tui` 命令：提供轻量终端 dashboard，展示项目、stage、质量摘要、artifact integrity 摘要、artifact、最近事件和下一步命令建议，支持 watch 刷新，并可通过 action flag 检查 artifact、输出命令建议、进入 guided selector、运行 provider smoke test、触发 rerun 或 worker recovery
