...
如果最终视频包含视频流，`render-output.json` 还会包含 `visualQuality`。它通过 `ffmpeg blackdetect` 读取黑屏片段和黑屏占比；黑屏占比较高会写入 warning，几乎全黑会写入 error，并纳入项目级 `status` / `quality` 聚合。
...
- render output quality：ffmpeg render 会探测最终视频，记录视频流、音频流、时长、音频响度和黑屏烟测 diagnostics
