...
For deterministic smoke checks, use a temporary mock workspace instead of the repository's `.video-agent` directory. The repository workspace may be configured for a hosted profile such as `mimo`, which requires external credentials.

```sh
WORKSPACE="$(mktemp -d)/video-agent-workspace"
bun run dev init --workspace "$WORKSPACE"
```
...
| MCP full config | Clients that accept an `mcpServers` object | `bun run dev mcp --print-config --workspace "$WORKSPACE"` |
| MCP client preset | Named clients that accept the common full `mcpServers` object | `bun run dev mcp --print-config --client claude-desktop --workspace "$WORKSPACE"` |
| MCP server entry | Clients that ask for only one server entry | `bun run dev mcp --print-config --config-shape server --server-name video-agent-local --workspace "$WORKSPACE"` |
| MCP server-entry preset | Clients whose UI already supplies the server name | `bun run dev mcp --print-config --client server-entry --workspace "$WORKSPACE"` |
| MCP preset info | Confirm whether the preset returns full config or a server entry | `bun run dev mcp --print-config-info --client server-entry --workspace "$WORKSPACE"` |
...
These commands should remain valid from the repository root:
...
bun run dev mcp --print-config --client server-entry --workspace "$WORKSPACE"
bun run dev mcp --print-config-info --client server-entry --workspace "$WORKSPACE"
bun run dev mcp --list-client-presets --workspace "$WORKSPACE"
bun run dev mcp --print-config --config-mode installed --workspace "$WORKSPACE"
bun run dev mcp --print-config --config-shape server --server-name video-agent-local --workspace "$WORKSPACE"
```

With a temporary mock workspace, the doctor surface reports a writable workspace, readable config, mock ASR/VLM/TTS providers, project count, `ffmpeg`, and `ffprobe`. The provider smoke test returns successful ASR/VLM/TTS contract checks. The MCP config surfaces produce:

- full `mcpServers.video-agent` config using `bun run dev mcp --workspace "$WORKSPACE"`
- named full-config presets for clients such as Claude Desktop and Cursor, which use the common `mcpServers` JSON shape
- installed `mcpServers.video-agent` config using `vagent mcp --workspace "$WORKSPACE"`
