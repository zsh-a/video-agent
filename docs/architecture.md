...
The existing root oclif CLI remains the first adapter during the initial scaffold. It includes a lightweight `vagent tui` terminal dashboard over runtime state plus controlled rerun and worker recovery actions; a later migration can move it into `apps/cli` or split a richer Ink workstation into `apps/tui` when the core commands are ready.
...
4. Add richer TUI interactions for artifact opening and guided command selection.
